...
HackRFSource::HackRFSource(RingBufPtr ring_buf)
    : ring_buf_(ring_buf) {}
...
bool HackRFSource::init(double center_freq_hz, uint32_t sample_rate,
    ...
    if (result != HACKRF_SUCCESS) {
        std::cerr << "hackrf_init() failed: "
                  << hackrf_error_name(static_cast<hackrf_error>(result)) << std::endl;
        return false;
    }
...
bool HackRFSource::start() {
    ...
    if (result != HACKRF_SUCCESS) {
        std::cerr << "hackrf_start_rx() failed: "
                  << hackrf_error_name(static_cast<hackrf_error>(result)) << std::endl;
        running_ = false;
        return false;
    }
...
void HackRFSource::set_gain1(uint32_t gain_db) {
    lna_gain_ = gain_db;
    if (device_)
        hackrf_set_lna_gain(device_, gain_db);
}

void HackRFSource::set_gain2(uint32_t gain_db) {
    vga_gain_ = gain_db;
    if (device_)
